...
Unsupported mechanics are not faked. Examples include fishing without an NPC-owned hook adapter, elytra without server-side NPC physics, resource-pack client flows on a server-side NPC, sleep/respawn flows that require vanilla `ServerPlayer` lifecycle state, enchanting/anvil/villager sessions without reviewed server-side adapters, dynamic pathfinder goals without a tick-follow adapter, and movement profiles that do not affect reviewed navigation.
...
- Inventory/equipment: `inventory`, `inventory_query`, `equip`, `equip_item`, facade-only `set_quick_bar_slot`, `unequip`, `move_slot_item`, `armor_manager_equip_best`, and `toss_stack`, `toss`, `drop_item`, `pickup_items_nearby`.
...
- Recipe queries and sessions: `recipes_for` and `recipes_all` query the server recipe manager. Loaded reachable block-entity `open_block`, `open_container`, `open_chest`, `window_deposit`, `window_withdraw`, `window_close`, `close_window`, and JSON-option `transfer` are live executor tools with NPC-owned session state and no-loss snapshot/rollback semantics. Transfer JSON options must include a supported direction, item type, and integer count from 1 to 256. Generic `window_deposit`, `window_withdraw`, and `transfer` reject furnace sessions with `use_furnace_specific_transfer_tools`; loaded reachable furnace-like blocks must use `open_furnace`, `furnace_status`, exact smeltable input insertion, exact valid-fuel insertion, and exact input/fuel/output withdrawal without fake smelting completion. `craft` remains unsupported until no-loss recipe consumption, remainders, and workstation validation are production-wired end to end.
- Bed/session queries: `is_bed` is a loaded block query. `sleep`, `wake`, and `respawn` remain explicit missing adapters for the server-side NPC lifecycle.
