...
OpenPlayer now has a focused selected-character conversation loop in `common` that reuses the existing runtime `IntentParser`; it does not add dependencies or a second provider path. The default runtime remains offline and safe. When provider endpoint, model, or API key is not configured, configured character conversation reports `Conversation unavailable: intent parser disabled`, does not call the parser/provider, and does not execute fallback actions.
...
Every loaded local character has a deterministic default assignment with assignment id equal to the character id unless an explicit same-id assignment targets that same character. Client list rows remain character-oriented for display but include both assignment id and character id; selected actions send only the stable assignment id to the server. If no assignment id is selected, target actions remain disabled and networking rejects the missing target.
...
Each implementation phase should be independently buildable and should avoid claiming parity for behavior that is unavailable or only implemented for one loader.
