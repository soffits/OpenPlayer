...
| `window.deposit`, `window.withdraw`, `window.close`, `openContainer`, `openChest` | `window_deposit`, `window_withdraw`, `window_close`, `open_container`, `open_chest` | `implemented_with_server_side_semantics` | The live executor uses an NPC-owned bounded server-side session for loaded reachable block-entity containers, so sessions survive per-submit executor replacement and close clears them. Transfers use no-loss snapshots and rollback on failed fit or missing item; furnace sessions are rejected so callers must use furnace-specific slot tools; unsupported/custom non-container blocks are rejected truthfully. |
| Furnace, enchantment table, anvil, villager APIs | `open_furnace`, `furnace_*`, `open_enchantment_table`, `enchantment_*`, `open_anvil`, `anvil_combine`, `open_villager`, `villager_*` | `implemented_with_server_side_semantics` / `unsupported_missing_adapter` | Loaded reachable furnace-like block entities support session status, smeltable input insertion, valid-fuel insertion, and exact input/fuel/output withdrawal. Smelting completion is not faked. Enchanting, anvil, and villager mutation/query surfaces still require real session adapters and remain rejected. |
...
| `sleep`, `isABed`, `wake`, `respawn` | `sleep`, `is_bed`, `wake`, `respawn` | `implemented_with_server_side_semantics` / `unsupported_missing_adapter` | `is_bed` is a loaded block query. Sleep, wake, and respawn remain unsupported because the server-side NPC is not a vanilla `ServerPlayer` sleeping/respawn lifecycle participant. |
...
| auto-eat and armor-manager | `auto_eat_enable`, `auto_eat_disable`, `auto_eat_status`, `armor_manager_equip_best`, `armor_manager_status` | `implemented_with_server_side_semantics` / `unsupported_missing_adapter` | `armor_manager_equip_best` uses existing no-loss armor selection on the NPC inventory and `armor_manager_status` is a visible idle facade. Auto-eat remains unsupported until the mode has reviewed bounded tick behavior. |
