...
Runtime command execution goes through a small automation backend seam. The default backend is `vanilla`, which runs server-side tasks against `OpenPlayerNpcEntity` through vanilla Minecraft APIs. It supports `STOP`, `MOVE`, `LOOK`, `FOLLOW_OWNER`, `PATROL`, `COLLECT_ITEMS`, and no-mutation `REPORT_STATUS`, plus disabled-by-default local world/inventory/combat actions for `EQUIP_BEST_ITEM`, `DROP_ITEM`, `BREAK_BLOCK`, `PLACE_BLOCK`, `ATTACK_NEAREST`, and `GUARD_OWNER`.
