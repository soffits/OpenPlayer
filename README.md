...
Runtime command execution goes through a small automation backend seam. The default backend is `vanilla`, which runs server-side tasks against `OpenPlayerNpcEntity` through vanilla Minecraft APIs. It supports `STOP`, `MOVE`, `LOOK`, `FOLLOW_OWNER`, `PATROL`, `COLLECT_ITEMS`, and no-mutation `REPORT_STATUS`, plus disabled-by-default local world/inventory/combat actions for `EQUIP_BEST_ITEM`, `EQUIP_ARMOR`, `USE_SELECTED_ITEM`, `SWAP_TO_OFFHAND`, `DROP_ITEM`, `BREAK_BLOCK`, `PLACE_BLOCK`, `ATTACK_NEAREST`, and `GUARD_OWNER`.

The vanilla backend is a focused NPC task layer, not full PlayerEngine or Baritone parity. `PATROL` accepts a strict `x y z` instruction within 32 blocks of the NPC and alternates between the start point and that coordinate with vanilla navigation. `REPORT_STATUS` returns deterministic local health, selected-slot, active-task, and queue information without mutating the world. `COLLECT_ITEMS` navigates to nearby visible item entities and relies on the NPC inventory pickup path when close, with a bounded close-range attempt so a full inventory cannot keep it active forever. Set `OPENPLAYER_AUTOMATION_ALLOW_WORLD_ACTIONS=true` or JVM property `openplayer.automation.allowWorldActions=true` to allow local world, inventory, and violent vanilla actions. When enabled, `EQUIP_BEST_ITEM` selects a useful hotbar weapon only when there is nearby valid combat context, and otherwise rejects without side effects. `DROP_ITEM` drops the selected hotbar stack near the NPC and clears that selected slot. `BREAK_BLOCK` accepts `x y z`, navigates near the loaded target block, rejects air or unbreakable blocks, selects the best scored tool from the NPC hotbar when one is available, swings the main hand, then uses server block destruction with drops. `PLACE_BLOCK` accepts `x y z`, uses the selected hotbar slot when it already contains a `BlockItem`, otherwise selects the first hotbar `BlockItem`, rejects occupied targets, checks simple vanilla survival and collision rules, swings the main hand, and places the block's default state when close. `ATTACK_NEAREST` accepts a blank instruction or radius number, avoids the owner, players, and other OpenPlayer NPCs, swings the main hand, and attacks the nearest visible living target within the original command radius while stopping if the NPC leaves that bound. `GUARD_OWNER` accepts a blank instruction or radius number, stays near the owner, and attacks only nearby visible hostile entities close to the owner. These tasks are bounded by simple radii and reach checks, but they are not fully deterministic pathfinding plans and do not include full inventory rearrangement, crafting, container interaction, digging pathfinding, item-use callbacks that require a real `Player`, block/entity interact routing, cooldown tracking, or complete automation parity.
