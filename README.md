...
Selecting a row shows its display name, assignment id, character id, description, skin status, lifecycle status, conversation status, recent spoken status lines, and action hints. Lifecycle status is resolved by the server-side companion manager from the active runtime session and reports `despawned` when no matching owner plus stable assignment id session exists. Spawn, despawn, follow, stop, and command text target that selected assignment only. With no character selected, target actions stay disabled.
...
The Status tab remains safe and presence-only for automation backend, parser state, endpoint host, model, API key status, source labels, and local character file operation results. It also shows bounded viewer/world capability diagnostics for the current status requester, including current dimension as an arbitrary ResourceLocation and implemented, diagnostic-only, or missing-adapter capability ids. These lines are not selected-NPC inventory or hidden queued execution; selected-NPC runtime status is reported only through the selected runtime path. OpenPlayer does not encode a hardcoded stronghold, End portal, dragon, or speedrun route tree in Java. The provider plans in normal Minecraft terms but may emit only reviewed generic primitives or truthful `UNAVAILABLE`/`REPORT_STATUS` diagnostics when adapters are missing. Character list entries report conversation as `not configured`, `unavailable: parser disabled`, or `available`; spoken status lines are server-authored bounded strings such as deterministic greetings, sanitized player messages, accepted intent summaries, or safe failures. They never include provider credentials, absolute paths, raw provider responses, or raw model output.
...
OpenPlayer also documents a local strategy/meta pack concept in `docs/strategy-meta-packs.md`. This is docs-only right now: the runtime does not auto-discover, load, parse, execute, or sync strategy packs. Strategy text is advisory reference for an LLM or planner, while Java remains responsible for reviewed primitives, capability adapters, validation, monitoring, and truthful status. Built-in examples may describe vanilla goals; modpack authors can add their own local strategy guidance and request/add adapters for modded mechanics without changing OpenPlayer core route logic.
...
The vanilla task layer is an adapter-backed NPC runtime, not a hardcoded product route planner. `PATROL`, `MOVE`, `LOOK`, `FOLLOW_OWNER`, and no-mutation `REPORT_STATUS` run through bounded vanilla navigation and local NPC state. Per-character `allowWorldActions=true` allows local world, inventory, and combat actions for that selected character only when a reviewed primitive/capability adapter exists, including active and passive item collection. World, inventory, and combat actions reject at execution time unless that NPC was spawned from a character with `allowWorldActions=true`.
...
- Continue broad local/offline player-like runtime extension work through documented capability gaps, reviewed adapters, and local strategy/meta guidance.
- Expand NPC-backed automation beyond the current vanilla task layer only through clean primitive, pathfinding, and action adapter boundaries.
