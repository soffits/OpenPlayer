...
public final class OpenAiCompatibleIntentProvider implements IntentProvider {
    ...
    static String systemPrompt() {
        return "Parse the user command for an OpenPlayer NPC. Return only compact JSON with string fields kind, priority, and instruction. "
            ...
            + "For PAUSE, UNPAUSE, and RESET_MEMORY, instruction must be blank. PAUSE stops automation ticks without clearing active or queued tasks; UNPAUSE resumes them; RESET_MEMORY clears only bounded local conversation history for the selected owner and assignment and does not delete character files, assignment files, provider config, logs, raw traces, or secrets. For BODY_LANGUAGE, instruction must be blank, idle, wave, swing, crouch, uncrouch, or look_owner. BODY_LANGUAGE is visual-only and must not be used for movement, pathing, inventory, world mutation, or combat; unsupported gestures such as nod and shake should use UNAVAILABLE. "
            + "For FARM_NEARBY, instruction must be blank or only a positive radius number. It scans only already-loaded nearby blocks with bounded radius, harvests one mature crop at a time, and replants only when server block/item metadata exposes a same-block replant capability and the needed item is in NPC normal inventory or hotbar. For FISH, instruction must be blank, a positive duration in seconds, stop, or cancel, but current NPC backend rejects actual fishing until a safe player-bound hook adapter exists; do not choose FISH unless the user explicitly asks to test fishing support. "
            ...
            + "Advanced world tasks are mostly unsupported and review-gated. LOCATE_LOADED_BLOCK, LOCATE_LOADED_ENTITY, and FIND_LOADED_BIOME are report-only loaded-world reconnaissance intents with instruction <resource_id> [radius]; they do not navigate, mutate the world, load chunks, or run long-range vanilla locate searches. LOCATE_STRUCTURE, EXPLORE_CHUNKS, USE_PORTAL, TRAVEL_NETHER, LOCATE_STRONGHOLD, and END_GAME_TASK are recognized only to return deterministic unsupported status until separate reviewed safe phases exist; do not choose Nether, End, dragon, stronghold, portal, structure-locate, speedrun, or chunk-exploration intents unless the user explicitly asks to test unsupported status. "
