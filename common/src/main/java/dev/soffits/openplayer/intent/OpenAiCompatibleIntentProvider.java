...
public final class OpenAiCompatibleIntentProvider implements IntentProvider {
    ...
    static String systemPrompt() {
        return "Parse the user command for an OpenPlayer NPC. Return only compact JSON with string fields kind, priority, and instruction. "
            ...
            + "Advanced world tasks are capability-clustered and review-gated. LOCATE_LOADED_BLOCK, LOCATE_LOADED_ENTITY, and FIND_LOADED_BIOME are report-only loaded-world reconnaissance intents with instruction <resource_id> [radius]; they do not navigate, mutate the world, load chunks, or run long-range vanilla locate searches. EXPLORE_CHUNKS is loaded-only bounded navigation; instruction must be blank, reset, clear, or key/value syntax radius=<blocks> steps=<count>, uses only already-loaded chunks near the NPC, never generates chunks, never teleports, and may truthfully fail when no safe loaded target exists. LOCATE_STRUCTURE is a loaded-only structure evidence diagnostic scan with instruction <structure_id> [radius] [source=loaded]; it supports only reviewed loaded-world evidence such as minecraft:village, returns source=loaded_scan evidence_found/not_found/unsupported_structure, may include diagnostic-only nearby loaded chest/barrel hints with no ownership, loot, or structure membership guarantee, never uses server locate APIs, never loads chunks, never teleports, never moves items, and never auto-loots. USE_PORTAL and TRAVEL_NETHER are player-like portal tasks; USE_PORTAL instruction must be blank or key/value syntax radius=<blocks> target=<minecraft:the_nether|minecraft:overworld> build=<true|false>, and TRAVEL_NETHER instruction must be blank or radius=<blocks> build=<true|false>. They may use existing loaded Nether portals and may build and ignite a simple portal only from NPC-carried obsidian plus flint_and_steel under strict validation; completion requires an observed dimension transition. They must not use OP/admin commands, teleport, /locate, /give, forced dimension changes, hidden services, opaque bot dependencies, arbitrary provider-origin code execution, or fake success. LOCATE_STRONGHOLD instruction must be blank or source=diagnostic and returns a stronghold task-tree diagnostic only; it must not claim triangulation until a reviewed eye-of-ender observation adapter exists. END_GAME_TASK instruction must be blank or one of plan, prepare, stronghold, portal, travel, dragon, or recovery and returns the visible task-tree diagnostic instead of monolithic speedrun success. "
            + "Portal and Nether resource preparation diagnostics are visible to the planner: REPORT_STATUS exposes portal origin dimension, target dimension, portal/frame position, observed transition, timeout/failure, and return affordance; Nether return travel should be requested through USE_PORTAL target=minecraft:overworld or TRAVEL_NETHER rather than teleport. Endgame preparation is a task tree over visible primitives: use REPORT_STATUS for dimension/hazards, TRAVEL_NETHER for travel, GET_ITEM for exact visible drops or recipes, ATTACK_TARGET for reviewed loaded hostile targets such as blazes/endermen, COLLECT_FOOD/SMELT_ITEM for food and safety preparation, and report missing primitives such as fortress search, barter/trade, or End orchestration instead of claiming success. "
