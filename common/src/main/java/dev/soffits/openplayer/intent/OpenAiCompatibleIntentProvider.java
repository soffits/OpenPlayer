...
public final class OpenAiCompatibleIntentProvider implements IntentProvider {
    ...
    static String systemPrompt() {
        return "Parse the user command for an OpenPlayer NPC. OpenPlayer may run you in an interactive tool loop: after each accepted, queued, completed, rejected, failed, timed out, or unavailable primitive observation, you may be called again with refreshed bounded runtime context. Return only compact JSON. For conversation or final response, use {\"chat\":\"message\",\"priority\":\"LOW\"|\"NORMAL\"|\"HIGH\"}. For refusal, use {\"unavailable\":\"reason\",\"priority\":\"LOW\"|\"NORMAL\"|\"HIGH\"}. "
            ...
            + "For compatibility only, a bounded plan may use {\"plan\":[{\"tool\":\"tool_name\",\"args\":{...}}],\"priority\":\"LOW\"|\"NORMAL\"|\"HIGH\"} with exactly 1 step; OpenPlayer expands it into the same interactive planner observations, and accepted or queued never means completed. Prefer one primitive tool per response when later observations may affect the next step. "
            ...
            + "Use args x, y, z only for move_to, look_at, break_block_at, dig, place_block_at, place_block, and activate_block. pathfinder_goto accepts a bounded goal object and is bridged only for coordinate goal shapes that existing loaded-area navigation can handle. move_to is coordinate-only; do not use owner, block id, entity id, search, resource, route, or goal syntax for move_to. Use empty args for pickup_items_nearby to collect any nearby dropped item, or args matching=<exact_item_id> and optional maxDistance to collect only matching nearby dropped items. Use empty args for report_status, inventory_query, stop, pause, and unpause; use args maxDistance for attack_nearest when needed. "
            + "For activate_block use args x, y, z. For activate_entity and use_on_entity use args entityId. These interaction primitives are player-like and capability-gated to loaded, reachable, line-of-sight targets with reviewed adapters. Unsupported custom blocks, unsupported entity interactions, position-specific entity activation, villager trading UI, breeding/taming without an adapter, and modded machines should use deterministic missing-adapter or UNAVAILABLE wording rather than fake success. For attack_target use args entityId and optional maxDistance; it only targets loaded explicitly allowlisted hostile danger entities and must not be used for players, owners, OpenPlayer NPCs, passive animals, friendly mobs, neutral mobs, or arbitrary non-hostile entities. "
            ...
            + "When an observation returns pos, recommended_next_pos, or recommended_next_action for find_loaded_blocks, use that exact returned coordinate for the matching next primitive such as break_block_at; do not substitute unrelated actionableTargets coordinates or invented positions. "
            ...
            + "High-level user goals must be solved by repeated bounded primitive tool calls and truthful observations; the Java runtime must not hide resource, workstation, smelting, farming, fishing, defending, building, exploring, locating, portal, Nether travel, End travel, stronghold, or speedrun/endgame macros. Decompose into the next currently supported primitive step, continue after completed observations, and change strategy after retryable rejected, failed, unavailable, or timed-out observations when a safe alternative primitive or diagnostic report exists. If the required next primitive is unsafe, policy-denied, ambiguous, or missing a real adapter, return UNAVAILABLE or ask through chat instead of faking success. Final chat only when the requested work is done according to observations or when you are truthfully reporting a limitation. "
