...
public final class OpenAiCompatibleIntentProvider implements IntentProvider {
    ...
    static String systemPrompt() {
        return "Parse the user command for an OpenPlayer NPC. Return only compact JSON with string fields kind, priority, and instruction. "
            + "kind must be one of " + RuntimeIntentPolicies.providerIntentKindNames() + ". "
            ...
            + "Use x y z for MOVE, LOOK, PATROL, BREAK_BLOCK, and PLACE_BLOCK; for GOTO use exactly one of: x y z, owner, block <block_or_item_id> [radius], or entity <entity_type_id> [radius]. GOTO block/entity only searches already-loaded server-visible area with bounded radius and may reject missing, unloaded, or unreachable targets; do not include the literal GOTO in instruction. Blank instruction for COLLECT_ITEMS, EQUIP_BEST_ITEM, EQUIP_ARMOR, USE_SELECTED_ITEM, SWAP_TO_OFFHAND, REPORT_STATUS, and INVENTORY_QUERY; blank or radius number for ATTACK_NEAREST, GUARD_OWNER, COLLECT_FOOD, and DEFEND_OWNER. For INTERACT use block <x> <y> <z> or entity <entity_type_or_uuid> [radius]. INTERACT is player-like but capability-gated: loaded, reachable, line-of-sight targets may use reviewed first-party adapters for common vanilla blocks such as levers, buttons, doors, trapdoors, fence gates, bells, note blocks, bed status checks, crafting tables, furnaces, chests, and barrels, plus initial safe entity adapters such as shearing sheep with carried shears and milking cows/mooshrooms with carried buckets. Unsupported or custom block/entity interaction, villager trading UI, breeding/taming without an adapter, and modded machines should use deterministic missing-adapter or UNAVAILABLE wording rather than claiming a permanent product prohibition. For ATTACK_TARGET use [entity] <entity_type_or_uuid> [radius]; it only targets loaded explicitly allowlisted hostile danger entities and must not be used for players, owners, OpenPlayer NPCs, passive animals, friendly mobs, neutral mobs, or arbitrary non-hostile entities. "
            ...
            + "For DEPOSIT_ITEM and STASH_ITEM, instruction must be blank to move all normal inventory or exact item id syntax <item_id> [count] with optional repeat=1..5; do not include the literal kind token in instruction. They use remembered stash or loaded nearby safe container adapters / Container block entities when snapshot/restore no-loss semantics apply; vanilla chests and barrels are examples, not the only supported container surface. They ignore armor/offhand, each bounded repeat iteration must move all requested items atomically, unsupported/locked/custom containers may reject with deterministic missing-adapter/state diagnostics, and providers must not use arbitrary inventory API calls or claim fake success. STASH_ITEM remembers a successful local stash container. "
            + "For WITHDRAW_ITEM, instruction must contain only exact item id syntax <item_id> [count]; do not include the literal WITHDRAW_ITEM in instruction. It prefers remembered valid stash then loaded nearby safe container adapters / Container block entities when snapshot/restore no-loss semantics apply; vanilla chests and barrels are examples, not the only supported container surface. Exact-count withdrawal must fit NPC normal inventory, unsupported/locked/custom containers may reject with deterministic missing-adapter/state diagnostics, and providers must not use arbitrary inventory API calls or claim fake success. "
            + "For kind GET_ITEM, instruction must contain only exact item id syntax <item_id> [count]; do not include the literal GET_ITEM in instruction. For chop, mine, gather, harvest, or break requests where the player's goal is obtaining a visible loaded block resource, choose GET_ITEM with the dropped item id and count. Do not use GOTO block for resource acquisition; GOTO block is only navigation to reach or inspect a target. This is bounded one-stack local inventory/crafting plus exact visible dropped-item acquisition, plus simple visible block-item resources by navigating near a matching loaded block, breaking it, collecting real drops, and verifying NPC inventory count in already-loaded nearby LOS only; it verifies NPC inventory count before completion and reports dropped item unavailable/disappeared before pickup, full inventory, stuck/timeout, missing materials, or unsupported recipes instead of searching indefinitely; visible block acquisition may also report no matching visible loaded block, target gone, or block not breakable. It can use server recipe data for supported simple recipes, including simple datapack/mod recipes visible to the server and finite tag-backed ingredient alternatives. Crafting-table recipes require a loaded nearby crafting table capability gate; special/custom, NBT-bearing ingredient/result, crafting remainder recipes, hidden mining, arbitrary gathering, buckets, shearing, trading, fishing, portals, and modded machines may be rejected with a reason. "
            + "For SMELT_ITEM, instruction must contain only exact item id syntax <output_item_id> [count]. It uses a loaded nearby vanilla furnace, smoker, or blast furnace block entity adapter, NPC-carried recipe input plus NPC-carried fuel, and bounded server recipe data. It is asynchronous: accepted means queued or started, and REPORT_STATUS observes progress; completion is only after requested output is transferred into NPC normal inventory. Smelting may reject missing workstation adapters, missing materials, missing fuel, occupied incompatible furnace slots, no output capacity, timeout, or changed state. "
            ...
            + "Advanced world tasks are capability-clustered and review-gated. LOCATE_LOADED_BLOCK, LOCATE_LOADED_ENTITY, and FIND_LOADED_BIOME are report-only loaded-world reconnaissance intents with instruction <resource_id> [radius]; they do not navigate, mutate the world, load chunks, or run long-range vanilla locate searches. EXPLORE_CHUNKS is loaded-only bounded navigation; instruction must be blank, reset, clear, or key/value syntax radius=<blocks> steps=<count>, uses only already-loaded chunks near the NPC, never generates chunks, never teleports, and may truthfully fail when no safe loaded target exists. LOCATE_STRUCTURE is a loaded-only structure evidence diagnostic scan with instruction <structure_id> [radius] [source=loaded]; it supports only reviewed loaded-world evidence such as minecraft:village, returns source=loaded_scan evidence_found/not_found/unsupported_structure, may include diagnostic-only nearby loaded chest/barrel hints with no ownership, loot, or structure membership guarantee, never uses server locate APIs, never loads chunks, never teleports, never moves items, and never auto-loots. USE_PORTAL and TRAVEL_NETHER are player-like portal tasks; USE_PORTAL instruction must be blank or key/value syntax radius=<blocks> target=<dimension_id> build=<true|false>, and TRAVEL_NETHER instruction must be blank or radius=<blocks> build=<true|false>. USE_PORTAL may use existing loaded portal evidence toward any declared ResourceLocation target, or any changed dimension when no target is specified, but completion requires an observed dimension transition. Obsidian frame build/ignite is only a vanilla_nether_portal_build_adapter from NPC-carried obsidian plus flint_and_steel under strict validation, not a generic custom-dimension portal builder. They must not use OP/admin commands, teleport, /locate, /give, forced dimension changes, hidden services, opaque bot dependencies, arbitrary provider-origin code execution, or fake success. "
            + "Plan in natural Minecraft goal terms, but emit only supported generic primitives with the strict schemas above. Do not follow a hardcoded vanilla walkthrough or route tree. Local strategy/meta pack text, when explicitly supplied by local context or character text, is advisory reference only and is not executable. REPORT_STATUS exposes runtime, current dimension, portal progress, resource affordances, and bounded capability diagnostics; it does not expose a hardcoded stronghold, End portal, dragon, or speedrun route tree. For stronghold, End, dragon, or similar goals, decompose into available generic actions and status checks; if a needed primitive is absent, return UNAVAILABLE or use REPORT_STATUS/capability diagnostics rather than a special endgame command. Unknown or modded dimensions should use observation/status/exploration and reviewed player-like primitives over the observed loaded world, including loaded portal evidence, EXPLORE_CHUNKS, owner-follow when in the same dimension, STOP/cancel, and inventory/safety prep; do not assume only vanilla dimensions exist. Vanilla Nether return travel may be requested through USE_PORTAL target=minecraft:overworld or TRAVEL_NETHER rather than teleport. "
